...
import Comment_container from "../components/comment";
import L_loader from '../components/L_loader'
import api from '../config/axios'
import Emoji_btn from "../components/emoji_btn";
import { Ten } from "./ten_slice";

export const BringComments = createAsyncThunk(
    "CommentSlice/BringComments",
    async (comments_ref, { dispatch, getState, rejectWithValue }) => {
        try {
            dispatch(open_comments())
            // dispatch(start_loading())
            const res = await api.get("/interaction/get_article_comments", {
                params: {
                    articleId: comments_ref.articleId,
                    collection_ref: comments_ref.collection_ref,

                }
            });
            // dispatch(stop_loading());
            return { comments: res.data, article: comments_ref }
...
export const Add_comment = createAsyncThunk(
    "CommentSlice/add_comment",
    async (comment, { dispatch, rejectWithValue }) => {
        return new Promise(async (resolve, reject) => {
            try {
                const { articleId, collection_ref, content, ownerId } = comment
                const res = await api.post("/interaction/add_comment", {
                    articleId, collection_ref, content, ownerId
                });
                dispatch(Ten([, "Comment submitted successfully."]))
                resolve(res.data)
            } catch (error) {
                dispatch(Ten([false, "Failed to  submit the comment"]))
...
const CommentSlice = createSlice({
    ...
    reducers: {
        ...
        builder.
            addCase(BringComments.pending, (state) => { state.isLoadingCmnts = true })
            .addCase(BringComments.fulfilled, (state, action) => {
                state.article = action.payload.article;
                state.comments = action.payload.comments
                state.isLoadingCmnts = false
            })
            .addCase(BringComments.rejected, (state, action) => {
                state.isLoadingCmnts = false
            })
            // ---

            // .addCase(Add_comment.pending, (state) => {
            // })
...
export const { close_comments, open_comments } = CommentSlice.actions

// ----------------------
export function Comments_page() {
    const { comments, article, isLoadingCmnts } = useSelector(s => s.Comments);
    const ContCmtsRef = useRef();
    const list_cmntRef = useRef();
    const dispatch = useDispatch();
    const { profile_img } = useSelector(s => s.User)
    const [isOpenToWriteCmnt, SetOpenToWriteCmnt] = useState(true);
    ...
    const handelSendComments = async () => {
        setSendingCmnt(true);
        const res_cmnt = await dispatch(Add_comment({
            ...article, content,
        }))
        setSendingCmnt(false);
        if (res_cmnt) {
            set_NoComments(false)
            setcontent('')
            SetOpenToWriteCmnt(false)
            list_cmntRef.current?.scrollTo({
                top: "0",
                behavior: "smooth",
            })
        }
    ...
    useEffect(() => {
        if (!isLoadingCmnts && comments.length == 0) {
            set_NoComments(true)
        }

    }, [isLoadingCmnts])

    return ReactDON.createPortal(
        <div className="backendMer"
            onClick={e => _OnContainerClickOut(e, ContCmtsRef.current, () => dispatch(close_comments()))}
        >
            <Dev
                ref={ContCmtsRef}
                initial={{
            ...
            >
...

                {
                    isLoadingCmnts &&
                    [0, 6, 4].map(() =>
                        <div className="c-s-s mt50 wmia pl20">
                            <div className="r-s-s">
                ...
                >
                    {
                        NoComments ?
                            ...
                            comments.map((c, index) =>
                                <Comment_container comment={c} delay={index * 0.1} key={c._id} />
                            )
                    }
                </div>
                    ...
                    <AnimatePresence>
                        {
                            ...
                            <motion.div
                                ...
                                className="createComment mt20 wmia r-b-c">
                                ...
                                <div className="r-b-c bg-third p5 br20 wmia">
                                    <textarea
                                        ...
                                        value={content}
                                        onChange={handelWriteComment}
                                        type="text" className="ml10 wmia" placeholder="What are your thoughts?"
                                    ></textarea>
                                    <div className="r-e-c">
                                        <Emoji_btn />
                                                ...
                                                <>
                                                    {
                                                        ...
                                                        <motion.button
                                                            ...
                                                            onClick={() => SetOpenToWriteCmnt(false)}
                                                            className='hoverEff2'>
                                                            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#FFFFFF"><path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z" /></svg>
                                                        </motion.button>
                                                    }
                    ...
                    <AnimatePresence>
                        {
                            ...
                            <motion.button
                                ...
                                onClick={() => SetOpenToWriteCmnt(true)}
                                className="w200 p10 br20 bg-third r-s-c">
                                ...
                                <p className="ml10">Write a comment</p>
                            </motion.button>
                        }
                    </AnimatePresence>
                </div>
